...
import { updateTradeType1, updateTradeType2, clearAllTrades, updatePerExpiryDate } from 'redux/slices/strategy';
...
export default function Slider({ open, setOpen }: props) {
  ...
  const getData = async (expiryDate: string, stockItem: string) => {
    ...
    try {
      ...
      dispatch(updatePerExpiryDate(data.per_expiry_data));
    } catch (error) {
  ...
  }, [expiryOption.value, activeStock.value, memoizedGetData]);
